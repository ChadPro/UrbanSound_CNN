...
def cnn_net(inputs, \
    ...
    with tf.name_scope(scope):
        net = conv2d_block(inputs, [3,3,1,32], [1,2,2,1], regularizer, scope="conv1")
        net = conv2d_block(net, [3,3,32,32], [1,1,1,1], regularizer, scope="conv2")
        net = conv2d_block(net, [3,3,32,32], [1,2,2,1], regularizer, scope="conv3")
        net = conv2d_block(net, [3,3,32,64], [1,1,1,1], regularizer, scope="conv4")
        net = conv2d_block(net, [3,3,64,64], [1,1,1,1], regularizer, scope="conv5")
        net = conv2d_block(net, [3,3,64,64], [1,1,1,1], regularizer, scope="conv6")
        net = conv2d_block(net, [3,3,64,64], [1,2,2,1], regularizer, scope="conv7")
